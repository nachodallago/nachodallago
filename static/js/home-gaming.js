...
function getSteamInfo(){
    ...
    .then(data => {
        console.log(data)
        if(data.status=='online'){statusTxt=' <span class="badge bg-success"><div class="gaming-online d-inline-block"></div> <span class="text-white">En línea</span></span>'}
        if(data.status=='in-game'){statusTxt='<span class="badge bg-success"> <div class="text-white"><div class="mb-2"><span class="gaming-online d-inline-block"></span> <span style="font-size:10px;font-weight:300">AHORA</span></div> <small> <img src="'+data.in_game.icon+'" class="rounded"> Jugando a '+data.in_game.game+'</small></div></span>'}
        if(data.status=='offline'){statusTxt='<span class="badge bg-secondary shadow text-dark">Actualmente desconectado</span>'}
